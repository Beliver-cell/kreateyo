...
// FOMO widget type enum
export const fomoWidgetTypeEnum = pgEnum('fomo_widget_type', [
  'live_purchase_popup',
  'viewer_count',
  'stock_urgency',
  'countdown_timer',
...
// FOMO settings table - per business configuration
export const fomoSettings = pgTable('fomo_settings', {
  id: uuid('id').primaryKey().defaultRandom(),
  businessId: uuid('business_id').notNull().references(() => businesses.id, { onDelete: 'cascade' }),
  
  // Live Purchase Popup settings
  livePurchaseEnabled: boolean('live_purchase_enabled').default(true),
  livePurchaseFrequency: integer('live_purchase_frequency').default(30), // seconds between popups
  livePurchaseDuration: integer('live_purchase_duration').default(5), // seconds to show
  livePurchaseUseRealData: boolean('live_purchase_use_real_data').default(true),
  livePurchaseSimulatedNames: jsonb('live_purchase_simulated_names').default([]), // fallback names
  livePurchaseSimulatedLocations: jsonb('live_purchase_simulated_locations').default([]), // fallback locations
  
  // Viewer Count settings
  viewerCountEnabled: boolean('viewer_count_enabled').default(true),
  viewerCountMinimum: integer('viewer_count_minimum').default(1), // minimum to show
  viewerCountMultiplier: decimal('viewer_count_multiplier', { precision: 3, scale: 2 }).default('1.0'), // boost factor
  viewerCountShowOnProduct: boolean('viewer_count_show_on_product').default(true),
  viewerCountShowOnCart: boolean('viewer_count_show_on_cart').default(false),
  
  // Stock Urgency settings
  stockUrgencyEnabled: boolean('stock_urgency_enabled').default(true),
  stockUrgencyThreshold: integer('stock_urgency_threshold').default(10), // show when stock < this
  stockUrgencyMessage: text('stock_urgency_message').default('Only {count} left - order soon!'),
  stockUrgencyColor: text('stock_urgency_color').default('#ef4444'),
  
  // Countdown Timer settings
  countdownEnabled: boolean('countdown_enabled').default(false),
  countdownEndDate: timestamp('countdown_end_date'),
  countdownMessage: text('countdown_message').default('Sale ends in'),
  countdownShowOnProducts: boolean('countdown_show_on_products').default(true),
  countdownProductIds: jsonb('countdown_product_ids').default([]), // specific products or empty for all
  
  // Sold Counter settings
  soldCounterEnabled: boolean('sold_counter_enabled').default(true),
  soldCounterTimeWindow: integer('sold_counter_time_window').default(24), // hours
  soldCounterMinimum: integer('sold_counter_minimum').default(5), // minimum to show
  soldCounterMessage: text('sold_counter_message').default('{count} sold in the last {hours} hours'),
  
  // Cart Abandonment settings
  cartAbandonmentEnabled: boolean('cart_abandonment_enabled').default(false),
  cartAbandonmentDelay: integer('cart_abandonment_delay').default(60), // seconds before popup
  cartAbandonmentMessage: text('cart_abandonment_message').default('Your cart is waiting!'),
  ...
  flashSaleDiscount: integer('flash_sale_discount'), // percentage
  flashSaleMessage: text('flash_sale_message').default('Flash Sale - {discount}% off ends tonight!'),
  flashSaleBgColor: text('flash_sale_bg_color').default('#dc2626'),
  flashSaleTextColor: text('flash_sale_text_color').default('#ffffff'),
  
  // Global styling
  popupPosition: text('popup_position').default('bottom-left'), // bottom-left, bottom-right, top-left, top-right
  primaryColor: text('primary_color').default('#10b981'),
  
  createdAt: timestamp('created_at').notNull().defaultNow(),
  updatedAt: timestamp('updated_at').notNull().defaultNow(),
}, (table) => ({
  businessIdx: uniqueIndex('idx_fomo_settings_business').on(table.businessId),
}));

// FOMO activity log - track real purchases and views for real-time data
export const fomoActivityLog = pgTable('fomo_activity_log', {
  id: uuid('id').primaryKey().defaultRandom(),
  businessId: uuid('business_id').notNull().references(() => businesses.id, { onDelete: 'cascade' }),
  activityType: text('activity_type').notNull(), // 'purchase', 'view', 'add_to_cart'
  productId: uuid('product_id'),
  productName: text('product_name'),
  customerName: text('customer_name'), // First name only for privacy
  customerLocation: text('customer_location'), // City/region
  quantity: integer('quantity').default(1),
  createdAt: timestamp('created_at').notNull().defaultNow(),
}, (table) => ({
  businessIdx: index('idx_fomo_activity_business').on(table.businessId),
  typeIdx: index('idx_fomo_activity_type').on(table.activityType),
  createdAtIdx: index('idx_fomo_activity_created').on(table.createdAt),
}));

// FOMO analytics - track widget performance
export const fomoAnalytics = pgTable('fomo_analytics', {
  id: uuid('id').primaryKey().defaultRandom(),
  businessId: uuid('business_id').notNull().references(() => businesses.id, { onDelete: 'cascade' }),
  widgetType: fomoWidgetTypeEnum('widget_type').notNull(),
  impressions: integer('impressions').default(0),
  clicks: integer('clicks').default(0),
  conversions: integer('conversions').default(0),
  date: timestamp('date').notNull().defaultNow(),
}, (table) => ({
  businessWidgetIdx: index('idx_fomo_analytics_business_widget').on(table.businessId, table.widgetType),
  dateIdx: index('idx_fomo_analytics_date').on(table.date),
}));
